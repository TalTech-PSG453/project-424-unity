...
public class Perrinn424CarController : VehicleBase
{
    ...
    UseUDP useUDP = UseUDP.Yes;             //Toggle UDP connectivity On/Off

    public int ReceivingUDPPort = 2398;     //UDP port for data transmission TO Unity3d (Unity3d receives data on this port)
    float counter = 0;

    int SourcePort = 25000;                 // UDP source port for data transmission FROM Unity3d (Unity3d sends data FROM this port)
    string destinationIp = "172.20.82.125"; //destination IP for data transmission FROM Unity3d (for example IP adress of Matlab) (Unity3d sends data to this IP)
    int destinationPort = 55000;            //destination port for data transmission FROM Unity3d (for example port of Matlab) (Unity3d sends data to this port)
    //UDP SETINGS END
    ...
    [Range(-1,1)]
    public float autoThrotle;

    public bool autoSpeedMode;
    ...
    bool isStarted = true; //for udp receive
    bool matlabConnected;
    float receivedThrottle = 0.0f;
    float receivedBrakes = 0.0f;
    ...
    protected override void DoUpdateBlocks ()
    {
        ...
        {
            UdpClient udp;
            udp = new UdpClient(ReceivingUDPPort);
            while (true)
            {
                ...
                try {
                    parsedValue = float.Parse(returnData);
                    
                    if(parsedValue >= 0 && parsedValue <= 1){
                        receivedThrottle = parsedValue;
                        receivedBrakes = 0.0f;
                    } else if (parsedValue < 0) {
                        receivedThrottle = 0.0f;
                        receivedBrakes = parsedValue * -90; // 90 is probably maximal pressure in brake system
        ...
        if(isStarted && useUDP == UseUDP.Yes){
            Thread thread;
            Debug.Log("UDP is started");
        ...
        if(useUDP == UseUDP.Yes){   //Receiving data from UDP
            m_gear = 1;
            m_throttleInput = receivedThrottle;
            m_brakePressure = receivedBrakes;
        } else {                    //Using default settings with no UDP connectivity
            m_throttleInput = autoSpeedMode?autoThrotle:input.GetThrottleInput(throttlePosition);
        ...
        if(FrontWheelDrive)m_frontPowertrain.differentialOverride = (Powertrain.DifferentialOverride)settingsData[SettingsData.DifferentialLock];
        if(RearWheelDrive)m_rearPowertrain.differentialOverride = (Powertrain.DifferentialOverride)settingsData[SettingsData.DifferentialLock];
    }
    ...
    protected override void DoUpdateData ()
    {
        ...
        float engineRpm = MathUtility.MaxAbs((FrontWheelDrive?m_frontPowertrain.mgu.sensorRpm:0), (RearWheelDrive?m_rearPowertrain.mgu.sensorRpm:0));
        vehicleData[VehicleData.EngineRpm] = (int)(engineRpm * 1000.0f); 
        
        static void SendStringUDP(int srcPort, string dstIp, int dstPort, byte[] data)	 {
            using (UdpClient c = new UdpClient(srcPort))			
            c.Send(data, data.Length, dstIp, dstPort);
        }


        if(useUDP == UseUDP.Yes && matlabConnected == true){
            if(counter < 0.05){
                counter += 0.002f;
            } else {
                string message = speed +"\r\n";
                //SendStringUDP(SourcePort, destinationIp, destinationPort, Encoding.ASCII.GetBytes(message));
                SendStringUDP(SourcePort, destinationIp, destinationPort, System.Text.Encoding.Default.GetBytes(message));
                counter = 0;
            }
        }
        ...
        float engineTorque = (FrontWheelDrive?m_frontPowertrain.mgu.sensorRotorTorque:0) + (RearWheelDrive?m_rearPowertrain.mgu.sensorRotorTorque:0);
        vehicleData[VehicleData.EngineTorque] = (int)(engineTorque * 1000.0f);

        // Engine Power: sum of the powers of both motors

        float enginePower = (FrontWheelDrive?m_frontPowertrain.mgu.sensorElectricalPower:0) + (RearWheelDrive?m_rearPowertrain.mgu.sensorElectricalPower:0);
        ...
        float engineLoad = ((FrontWheelDrive?m_frontPowertrain.mgu.sensorLoad:0) + (RearWheelDrive?m_rearPowertrain.mgu.sensorLoad:0)) * (AllWheelDrive?0.5f:1f);
        ...
        if(FrontWheelDrive)m_frontPowertrain.FillDataBus(customData, Perrinn424Data.FrontMguBase);
        if(RearWheelDrive)m_rearPowertrain.FillDataBus(customData, Perrinn424Data.RearMguBase);
